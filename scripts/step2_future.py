    ...
    for Scen in ['Continued','Accelerated','Pledged']:
        for Strategy in ['Linear']:
                for SSP in ['SSP1-2.6']:
                    ...
                    if n_run==1: For.to_netcdf(file_pathin + f'results/n{n_run}_test_grid_full_For_{Scen}_{Strategy}_{SSP}.nc', encoding={var:{'zlib':True, 'dtype':np.float32} for var in For})
    ...
    for Scen in ['Continued','Accelerated','Pledged']:
        for Strategy in ['Linear']:
                for SSP in ['SSP1-2.6']:
                    ...
                    For = xr.open_dataset(file_pathin + f'results/n1_test_grid_full_For_{Scen}_{Strategy}_{SSP}.nc', chunks={'config':5})
